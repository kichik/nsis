...
static void
unget (input * in, int c, filepos * pos)
{
 ...
 */
static int
kwcmp (wchar_t const *p, char const *q)
{
  int i;
  do
    {
 ...
 */
static word *
addword (word newword, word *** hptrptr)
{
  word *mnewword;
  if (!hptrptr)
    return NULL;
 ...
 */
static paragraph *
addpara (paragraph newpara, paragraph *** hptrptr)
{
 ...
 */
static void
read_file (paragraph *** ret, input * in, indexdata * idx)
{
  token t;
  paragraph par;
...
paragraph *
read_input (input * in, indexdata * idx)
{
  paragraph *head = NULL;
  paragraph **hptr = &head;
