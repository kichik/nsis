...
int CEXEBuild::write_output(void)
{
  ...
  // JF> datablock randomizer -- this can produce 0.5-1.5% differences in file sizes with LZMA in solid mode
  if (1 && build_compress_whole && build_datablock.getlen() && build_datablock.getlen() < 256*1024*1024) //randomly reorder datablock chunks!
  {
    int r=GeneticImproveDatablock(100,2);
    if (r != PS_OK) 
    {
      fclose(fp);
      return r;
...
#ifdef NSIS_CONFIG_COMPRESSION_SUPPORT
int CEXEBuild::deflateToNowhere(INT64 *outcnt, char *buf, int len) // len==0 to flush
{
  build_compressor_set=true;
  ...
  for (;;)
  {
    ...
    {
      ERROR_MSG("Error: deflateToNowhere: deflate() failed(%s [%d])\n", compressor->GetErrStr(ret), ret);
      return 1;
    }
...
#ifdef NSIS_CONFIG_COMPRESSION_SUPPORT
int CEXEBuild::GeneticImproveDatablock(int ngenerations, int min_children)
{
  if (!build_compress_whole||!build_datablock.getlen()) return PS_OK;

  build_datablock.setro(FALSE);

  TinyGrowBuf chunks;

  struct DataBlockChunk
  {
    //double weight,parent_weight, bestchild_weight;
    TinyGrowBuf data;
    int orig_pos, new_pos;

    int csizeest;

    static int chunkSortFuncSz(const void *a, const void *b)
    {
      DataBlockChunk *aa = *(DataBlockChunk**)a;
      DataBlockChunk *bb = *(DataBlockChunk**)b;
      int d=(aa->data.getlen()-bb->data.getlen());
      if (d) return d;
      return memcmp(bb->data.get(),aa->data.get(),aa->data.getlen());
    }
    static int chunkSortFuncSz2(const void *a, const void *b)
    {
      DataBlockChunk *aa = *(DataBlockChunk**)a;
      DataBlockChunk *bb = *(DataBlockChunk**)b;
      int d=- (aa->data.getlen()-bb->data.getlen());
      if (d) return d;
      return memcmp(aa->data.get(),bb->data.get(),aa->data.getlen());
    }
    static int chunkSortFuncRatio(const void *a, const void *b)
    {
      DataBlockChunk *aa = *(DataBlockChunk**)a;
      DataBlockChunk *bb = *(DataBlockChunk**)b;

      double ratio1 = (aa->data.getlen()) / (double) aa->csizeest;
      double ratio2 = (bb->data.getlen()) / (double) bb->csizeest;
      if (ratio1-ratio2<0.0)return 1;
      if (ratio1-ratio2>0.0) return -1;

      return 0;
    }
    static int chunkSortFuncRatio2(const void *a, const void *b)
    {
      DataBlockChunk *aa = *(DataBlockChunk**)a;
      DataBlockChunk *bb = *(DataBlockChunk**)b;
  ...
  int left=build_datablock.getlen();
  char *dptr=(char*)build_datablock.get(0,left);
  char *ptr = dptr;
  MTRand rng;
  while (left>=sizeof(int))
  {
    int thislen = *(int*)ptr;
    if (left < thislen+(int)sizeof(int)) break;

    DataBlockChunk *p=new DataBlockChunk;
    p->data.add(ptr,thislen+sizeof(int));
    p->new_pos = p->orig_pos=ptr-dptr;      
    chunks.add(&p,sizeof(void*));

    ptr += thislen+sizeof(int);
    left -= thislen+sizeof(int);
  }

  int chunkcount = chunks.getlen()/sizeof(void*);
  DataBlockChunk **chunklist = (DataBlockChunk **)chunks.get();


  int x;
  if (left==0) // correctly parased the whole datablock
  {
    TinyGrowBuf sortspace;
    sortspace.resize(chunks.getlen());

    INFO_MSG("RANDOMIZER: working with %d blocks...\n",chunkcount);

    struct FixUpType
    {
      int *fix;
      DataBlockChunk *chunk;
    };
    TinyGrowBuf fixup_addresses;

    for (x=0;x<chunkcount;x++)
    {
      int oldpos = chunklist[x]->orig_pos;

      // convert oldpos to newpos
      entry *ent = (entry *) build_entries.get();
      int ents = build_header.blocks[NB_ENTRIES].num;
      if (ent) while (ents--)
      {
        if (ent->which == EW_WRITEUNINSTALLER)
        {
          if (ent->offsets[1] == oldpos) 
          { 
            FixUpType ft={&ent->offsets[1],chunklist[x]};
            fixup_addresses.add(&ft,sizeof(ft));
          }
        }
        else if (ent->which==EW_EXTRACTFILE)
        {
          if (ent->offsets[2]==oldpos) 
          { 
            FixUpType ft={&ent->offsets[2],chunklist[x]};
            fixup_addresses.add(&ft,sizeof(ft));
          }
        }
        else if (ent->which==EW_WRITEREG && ent->offsets[4] == REG_BINARY && ent->offsets[5] == REG_BINARY)
        {
          if (ent->offsets[3]==oldpos) 
          { 
            FixUpType ft={&ent->offsets[3],chunklist[x]};
            fixup_addresses.add(&ft,sizeof(ft));
          }
        }
        ent++;
    ...
    int num_fixups = fixup_addresses.getlen()/sizeof(FixUpType);
    INFO_MSG("Instructions that will need fixups: %d\n",num_fixups);
    FixUpType *fixups = (FixUpType *)fixup_addresses.get();

    double wscale=0.5;

    INT64 baseline=0;

    TinyGrowBuf current_parent, bestchild_list;
    INT64 parent_size=0, bestchild_size=0;

    current_parent.resize(chunks.getlen());
    bestchild_list.resize(chunks.getlen());
    ...
    {
      if (parent_size)
        INFO_MSG("gen %d child %d: ",generation+1,child_count+1);
      else INFO_MSG("calculating master parent: ");
      ...
      if (parent_size)
      {
        ...
        {
          if (parent_size)
            memcpy(chunks.get(),current_parent.get(),current_parent.getlen());


          if (generation==0)
          {
            if (child_count==0)  WDL_mergesort(chunklist,chunkcount,sizeof(void *),DataBlockChunk::chunkSortFuncSz2,(char*)sortspace.get());
            else if (child_count==1) WDL_mergesort(chunklist,chunkcount,sizeof(void *),DataBlockChunk::chunkSortFuncSz,(char*)sortspace.get());
            else if (child_count==2)
            {
              WDL_mergesort(chunklist,chunkcount,sizeof(void *),DataBlockChunk::chunkSortFuncSz2,(char*)sortspace.get());
              ...
              for (x=0;x<chunkcount;x++)
              {
                ...
                int besty=0;
                for (y=x+1;y<x+3&&y<chunkcount;y++) // find next nearest block that compresses with us with highest ratio
                {
                  INT64 sz=0;
                  compressor->Init(build_compress_level, build_compress_dict_size);
                  deflateToNowhere(&sz,(char*)chunklist[x]->data.get(),chunklist[x]->data.getlen());
                  deflateToNowhere(&sz,(char*)chunklist[y]->data.get(),chunklist[y]->data.getlen());
                  deflateToNowhere(&sz,NULL,0);
                  compressor->End();

                  double ratio = (chunklist[x]->data.getlen()+chunklist[y]->data.getlen())/(double)sz;
                  if (ratio > bestratio) { bestratio=ratio; besty=y; }
                }
                if (besty>x+1)
                {
                  DataBlockChunk *a=chunklist[x+1];
                  chunklist[x+1]=chunklist[besty];
                  chunklist[besty]=a;
                }
              }
            }
          ...
          {
            int a1=rng.randInt()%chunkcount;
            int a2=((child_count&1) ? a1+1 : rng.randInt())%chunkcount;
            if (a1 !=a2)
            {
              DataBlockChunk *a=chunklist[a1];
              chunklist[a1]=chunklist[a2];
              chunklist[a2]=a;
            }
          }

          if (!memcmp(chunklist,current_parent.get(),current_parent.getlen()))
          {
            continue;
          }
          break;
        ...
        if (chcnt<=0)
        {
          INFO_MSG("list cant seem to change!\n");
          break;
        }
      }
      ...
      for (x=0;x<chunkcount;x++) // calculate new addresses
      {
        chunklist[x]->new_pos = ptr-dptr;
        ptr+=chunklist[x]->data.getlen();

      }

      // fixup instructions
      for(x=0;x<num_fixups;x++) *fixups[x].fix = fixups[x].chunk->new_pos;

      // try compressing
      int n = compressor->Init(build_compress_level, build_compress_dict_size);
      if (n != C_OK)
      {
        ERROR_MSG("GeneticImproveDatablock: error #12345: deflateInit() failed(%s [%d]).\n", compressor->GetErrStr(n), n);
        return PS_ERROR;
      }
      ...
      INT64 size=0;
      deflateToNowhere(&size,(char*)ihd.get(),ihd.getlen());

      for (x=0;x<chunkcount;x++) // compress chunks
      {
        if (deflateToNowhere(&size,(char*)chunklist[x]->data.get(),chunklist[x]->data.getlen()))
        {
          return PS_ERROR;
        }
      }

      deflateToNowhere(&size,NULL,0);
      compressor->End();
      if (!size)
      {
        ERROR_MSG("compressed to 0! WTF\n");
        return PS_ERROR;
      }


      if (!parent_size) // save oldest info
      {
        baseline=parent_size=size;        
        memcpy(current_parent.get(),chunks.get(),chunks.getlen());
        child_count=0;
        bestchild_size=0;
        INFO_MSG("master parent size=%u, 100%%\n",(int)size);
      }
      else
      {
        if ((!bestchild_size || size<bestchild_size) && size < parent_size)
        {
          bestchild_size=size;
          // save best child info
          memcpy(bestchild_list.get(),chunks.get(),chunks.getlen());

          INFO_MSG("size=%u, wscale=%.4f, NEW BEST = %.5f%%\n",(int)size,wscale,size*100.0/baseline);         
        }
        else
          INFO_MSG("size=%u, wscale=%.4f\n",(int)size,wscale,size*100.0/baseline);

        if (++child_count >= (generation==0 ? 3 : min_children))
        {
          if (bestchild_size && bestchild_size<parent_size)
          {
            INFO_MSG("New generation, size=%u, wscale=%.4f, NEW BEST = %.5f%%\n",(int)bestchild_size,wscale,bestchild_size*100.0/baseline);

            //best child becomes parent
            parent_size=bestchild_size;
            memcpy(current_parent.get(),bestchild_list.get(),bestchild_list.getlen());

            wscale/=1.2;
            if (wscale<0.001) wscale=0.001;
          }
          else
          {
            INFO_MSG("Last generation deadended, increasing scale, parentsize=%u, wscale=%.4f, CUR BEST = %.5f%%\n",(int)parent_size,wscale,parent_size*100.0/baseline);
            wscale*=1.2;
    ...
    if (parent_size)
    {
      ...
      // use best list
      memcpy(chunks.get(),current_parent.get(),current_parent.getlen());
      for (x=0;x<chunkcount;x++) // build datablock
      {
        chunklist[x]->new_pos = ptr-dptr;
        memcpy(ptr,(char*)chunklist[x]->data.get(),chunklist[x]->data.getlen());
        ptr+=chunklist[x]->data.getlen();
