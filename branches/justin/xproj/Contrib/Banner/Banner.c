...
void __declspec(dllexport) getWindow(HWND hwndParent, int string_size, char *variables, nsis_stack_t **stacktop)
{
  wsprintf(buf, "%u", hwBanner);
  pushstring(buf);
}

void __declspec(dllexport) destroy(HWND hwndParent, int string_size, char *variables, nsis_stack_t **stacktop)
{
  if (!hwBanner)
    return;
