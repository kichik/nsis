...
static unsigned long context_hash(char *context)
{
    ...
    enum { assertion =
	    1 / (((unsigned long) 0xFFFFFFFF) + 2 == (unsigned long) 1)
    };
...
void whlp_end_para(WHLP h)
{
    ...
    memcpy(h->link->data1, h->linkdata1 + data1cut,
	   h->link->len1 - data1cut);
    memcpy(h->link->data1 + h->link->len1 - data1cut, h->linkdata1,
	   data1cut);
...
static void whlp_topic_layout(WHLP h)
{
	...
	topicnum++;
	if (link->nonscroll)
	    PUT_32BIT_LSB_FIRST(link->data1 + 16,
				link->nonscroll->topicpos);
	else
	...
	if (link->nexttopic)
	    PUT_32BIT_LSB_FIRST(link->data1 + 24,
				link->nexttopic->topicpos);
	else
